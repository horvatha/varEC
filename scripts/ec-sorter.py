...
#############################################################
 ...
 Actual information:
    http://www.arek.uni-obuda.hu/harp/latex/ec
...
import sys
sys.path.append('.')     #For ec-sorter.py
sys.path.append('./bin') #For fsor.py
## if sys.version[:1] < '2':
##     error('version')
...
from varEC.lang import lang, mesg, keys, dictionary, err
try:
...
from varEC.message import error, message, ask_exit, ask, menu, cls, get_integer

from varEC.books import Books, ExerciseBook

MYDIR = os.path.abspath(sys.path[0])
#BASEDIR =  os.path.join(MYDIR, 'base')
      ...
      # Files
        ...
        files.output = arg
          ...
          if os.path.isfile(arg):
              ...
              raise getopt.GetoptError('"%s" is not a valid filename' %arg)
      elif opt in ['-p', "--print"]:
          pass
    return args
...
def test_exercise_numbers():
  ...
  #int_codelist = [x for x in books.codelist if type(x) == type(1)]
  int_codelist = []
  for x in books.codelist():
    if type(x) == type(1):
      int_codelist.append(x)
...
def name_list(name, file_paths=None):
  ...
  if options.verbose>0: print(' name_list = %s' % name_list)
  return name_list
...
def make_testpapers():
  ...
  if options.interactive:
    ...
    files.setup(); cls()
  if not files.input:
    files.input=raw_input(ask('file'))
  ...
  if exercise_numbers and options.interactive:
    ...
    for group in exercise_numbers:
      for item in group[1:]:
        try: txt=int(item)
        except ValueError: txt = "%70s" % item
        print(txt)
  ...
  text_testpaper = varexercise.frame(text,
                                     doc_type = 'testpaper',
  ...
  if options.verbose > 0: message('file opened', file_name)
  f=open( file_name, 'w')
  f.writelines(text_testpaper)
  f.close()

  if solution_file:
    text_solution = varexercise.frame(solution_text,
                                      doc_type = 'plain',
                                      preamble_file = "magyarpreambulum",
    ...
    if options.verbose > 0: message('file opened', solution_file_name)
...
def _group_test():
  books = Books(['matematika_hu/analizis.tex','matematika_hu/vektor_matrix.tex'], 'exercise series')
  print(books)
...
bugs = """
     ...
     5. varexercise.VarExercise.one_variation() or  varexercise.eraser() calculates   badly if there are constants in the exercise.
...
nov 30.
1. The varexercise.make_tespapers() makes a var.tex file with the variation in latex table form in it.
...
1. The variation summary (var.tex) is in another form. It lists the values
            not into a table, just into a paragraph (with unit if this exists). I
...
1. The fesormessage is a new modul. Here deals with (almost) all the printing to the monitor.

2. There is a menu at the beginning. The new items are:
            Make code-numbers for the exercis,  if doesn't have. (Not ready yet)
