...
class TestHelperFunctions(unittest.TestCase):
    ...
    varsets_from_string = (
        ('t,d v0 t,v v0 t,alpha v0 v,E v', (frozenset('t'), frozenset(['d','v0','t']), frozenset(['v','v0','t']), frozenset(['alpha','v0','v']), frozenset(['E','v']))),
        ('t, d v0 t, v v0 t , alpha v0 v , E v', (frozenset('t'), frozenset(['d','v0','t']), frozenset(['v','v0','t']), frozenset(['alpha','v0','v']), frozenset(['E','v']))),
...
class TestMakeEquations(unittest.TestCase):
    ...
    def test_known_values(self):
        ...
        for formulas, variables, known_equations in self.formulas:
            self.assertEqual(possibilities.make_equations(formulas, variables), known_equations)
...
class TestPossibilities(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.known_values = {
                ...
                [[set(vars.strip().split()) for vars in calc_parts.split(',')] for calc_parts in raw_values[key]]
    ...
    def test_new_calculatable(self):
        "new_calculatable() should return proper values"
        for equations in self.known_values:
            for calc_parts in self.known_values[equations]:
                ...
                for i in range(len(calc_parts) - 1):
                    ...
                    self.assertEqual(possibilities.new_calculatable(calc, equations), calc_parts[i+1])
        ...
        for equations in self.known_values:
            ...
            for calc_parts in self.known_values[equations]:
                ...
                for part in calc_parts:
                    calc |= part
                self.assertEqual(possibilities.calculatable(calc_parts[0], equations, vars), calc)
    ...
    def test_too_many_protected(self):
        ...
        for protected_variables in [set(i) for i in 'abc def fegd acdeg'.split()]:
            self.assertRaises(AssertionError, possibilities.possibilities, equations, protected_variables)
...
def suite():
    ...
    return unittest.TestSuite([Possibilities_suite, TestGetVariables_suite, TestMakeEquations_suite, TestHelperFunctions_suite])
